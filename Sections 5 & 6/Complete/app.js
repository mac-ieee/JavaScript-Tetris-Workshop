...
// SECTION 5: DRAW(), UNDRAW(), FREEZE()
	...
	*/
	...
	function moveRight() {
		undraw()
		//tetromino is at right if the (position / width) has a remainder of the width-1
		const atRight = current.some(index => (currentPosition + index)%width === width - 1)
		if(!atRight){
			// moves position right by 1 if not at right edge
			currentPosition +=1
		}
		if(current.some(index => squares[currentPosition + index].classList.contains('taken'))) {
			// moves position left by 1 if there is a tetromino already there
			currentPosition -=1
		}
		draw()
	}
